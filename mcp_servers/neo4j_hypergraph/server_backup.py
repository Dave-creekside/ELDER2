...
# Lazy imports for sentence transformers - only import when needed
...
class Neo4jSemanticHypergraphServer:
    ...
    def setup_handlers(self):
        ...
        async def handle_list_tools() -> List[Tool]:
            ...
            return [
                ...
                Tool(
                    name="contextualize_graph",
                    description="Calculate semantic similarities for all concept pairs in the graph",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "min_similarity": {"type": "number", "description": "Minimum similarity to create edges", "default": 0.3}
                        }
                    }
                ),
                Tool(
        ...
        async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
            ...
            try:
                ...
                elif name == "contextualize_graph":
                    result = await self.contextualize_graph(
    ...
    async def create_hyperedge(self, members: List[str], label: Optional[str] = None, 
        ...
        """Create a semantic hyperedge connecting multiple concepts"""
        if len(members) < 2:
        ...
        async with self.driver.session() as session:
            ...
            # Calculate semantic weights and create connections
            total_weight = 0.0
            connection_count = 0
                ...
                for j, member2 in enumerate(members):
                    if i < j:  # Avoid duplicates
                        ...
                        if auto_calculate_weights:
                            weight = await self.get_semantic_similarity(member1, member2)
                        
                        # Create weighted semantic relationship
                        rel_query = """
                        MATCH (c1:Concept {name: $member1})
                        MATCH (c2:Concept {name: $member2})
                        ...
                        MERGE (c1)-[:MEMBER_OF]->(he)
                        MERGE (c2)-[:MEMBER_OF]->(he)
                        RETURN r
                        """
            ...
            # Update hyperedge with average weight
            avg_weight = total_weight / connection_count if connection_count > 0 else 0.0
            await session.run("""
                MATCH (he:Hyperedge {id: $hyperedge_id})
                SET he.avg_semantic_weight = $avg_weight
            """, hyperedge_id=hyperedge_id, avg_weight=avg_weight)
    ...
    async def contextualize_graph(self, min_similarity: float = 0.3) -> Dict[str, Any]:
        """Calculate semantic similarities for all concept pairs"""
        async with self.driver.session() as session:
            # Get all concepts
            result = await session.run("MATCH (c:Concept) RETURN c.name as name")
            concepts = [record["name"] async for record in result]
            
            if len(concepts) < 2:
                return {"success": True, "message": "Not enough concepts to contextualize"}
                ...
                for j, concept2 in enumerate(concepts):
                    if i < j:  # Avoid duplicates
                        ...
                        if similarity >= min_similarity:
                            # Create or update semantic relationship
                            rel_query = """
                            MATCH (c1:Concept {name: $concept1})
                            MATCH (c2:Concept {name: $concept2})
                            MERGE (c1)-[r:SEMANTIC]-(c2)
            ...
            return serialize_for_json({
                ...
                "similarities_calculated": similarities_calculated,
...
async def main():
    ...
    # Connect to Neo4j
    await server_instance.connect_neo4j()
